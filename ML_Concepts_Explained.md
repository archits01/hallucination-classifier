...
## 3. Dealing with Text: TF-IDF (Feature Extraction)
Machine Learning models are mathematical formulas. They don't understand English words like "banana" or "hallucination"; they only understand numbers. So, we have to convert our Question+Answer text into numbers.

**TF-IDF (Term Frequency - Inverse Document Frequency)** is the technique we used to do this. 
* **TF (Term Frequency):** How often does a specific word appear in this specific answer? (If the word "always" appears 5 times, it gets a high TF score).
* **IDF (Inverse Document Frequency):** But wait, words like "the", "is", and "and" appear frequently everywhere! IDF lowers the score of common words and boosts the score of rare, unique words.

**Result:** Every text example becomes a row of numbers, where each number represents the "importance" of a specific word in that text. We limited our model to the top 5,000 most important words (`max_features=5000`).
